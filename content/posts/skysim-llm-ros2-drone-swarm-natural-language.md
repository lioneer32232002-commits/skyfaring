...
tags:
  ...
  - 多代理人
  - 人工智慧
excerpt: "你說一句『排成一個半徑兩公尺的圓圈』，三十架無人機就排好了，沒有撞機，不需要任何專業知識。DAUS' 2026 的這篇論文展示了這件事。讓它成真的關鍵，是知道在哪裡不能信任 LLM。"
...
當你有三十架無人機，要讓它們同時移動到不同位置，LLM 需要為每一架都規劃一個 3D 座標，而且這些座標必須在物理上可行：不能飛出邊界、不能互相撞上、速度不能超過馬達上限。
...
收到自然語言命令，系統的第一件事是先訂閱 ROS2 的 `/odom` 話題，取得所有無人機的當前 3D 座標。
...
LLM 給的是靜態目標點，不是飛行路徑。把無人機從現在的位置移到目標位置，是執行層的工作，而執行層的核心是人工勢場（Artificial Potential Field，APF）控制器。
...
LLM 的空間推理能力確實令人意外地強，在沒有任何針對性訓練的情況下，Gemini 3.5 Pro 對「5×6 網格」、「3D 立方體」、「聖誕樹」這些抽象描述，都能翻譯出合理的 3D 座標。這個能力是「泛化的」，不需要事先定義每種形狀。
