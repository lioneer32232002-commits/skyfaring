...
方法是高斯混合模型（Gaussian Mixture Model，GMM）。不同於 k-means 的硬性分類，GMM 給出機率分佈。球員可以是「70% 的 Ball Dominant Scorer、30% 的 Stretch Forward」，而不是被強迫塞進單一框格。貝氏資訊準則（BIC）自動決定最佳群集數。
...
他們收集了同期的 20,000 筆五人陣容資料，用「貝氏調整後 Net Rating」（每 100 次攻守回合的得失分差）為目標，建立 Random Forest 模型，測試 310 萬種可能的陣容組合。
