...
Low Efficiency Defender（低效率防守者）：兩分出手少、命中率低，但犯規多。代表球員是 OG Anunoby。這類球員在場上的價值往往不會出現在統計數字裡，但也因此容易被高估潛力。
...
ANOVA 驗證了這 9 個群集的統計顯著性，13 項變項全數在群間達到顯著差異（p < 0.001），936 組兩兩配對比較中有 864 組達到顯著，說明這 9 個群集是數據裡真實存在的結構，不是硬切出來的分類。
...
Penner 在和加拿大國家隊教練討論時，教練明確提到某些非量化因素，資歷、更衣室影響力、球員願不願意配合戰術，這些東西不會出現在 box score 裡，但對臨場決策有真實影響。模型建議選 Brandon Clarke，教練選了 Khem Birch；模型建議選 Conor Morgan，教練選了 Cory Joseph。這些替換不算模型的錯誤，教練補上的是模型涵蓋不到的資訊。
...
如果你讀過我們介紹過的另一篇分群研究，[「控球後衛」這個標籤，已經是個落後的分類了。](/blog/nba-player-clustering-sloan-2020/)，你大概已經注意到，兩篇研究都找到了「9 種球員類型」這個答案。但兩篇研究問的問題並不相同。
