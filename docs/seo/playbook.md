...
| v1.1 | 2026-08-28 | 第四層新增第 19 項：文章標題與子標題照搜尋意圖設計（站主決策，配合第 15、16、17 項的既有結論） |
...
19. **標題與子標題照搜尋意圖設計**（站主決策 2026-08-28，並依第 16 項的段落自足結論）：
    - 主標題前段放使用者實際會搜的實體詞（機型、地名、機構、人名、事件），意象或懸念放後半；主標題長度約 30 個全形字以內，關鍵字擠在前 20 字
    - 同一篇盡量覆蓋兩個查詢群（例：航空專業詞 ＋ 旅遊地名），讓兩種讀者的搜尋都打得到同一頁
    - `##` 子標題自帶實體詞（人名、年份、機型、地名），不寫「首飛那天」這種無關鍵字短語；子標題等於整頁的內部大綱，也是 AI 引用時的切片邊界
    - 關鍵字排好之後，還要讓人想點進去（站主決策 2026-08-31）：標題要給讀者一個他想知道答案的問題，不是描述這篇文章做了什麼。「19 季數據看規則能不能壓住三分」是在講作業內容；「為了減少三分出手，歐洲聯賽三分線外移半公尺，有效嗎？」才是搜尋的人腦子裡真正的問題。下標最後一步唸一次標題，問自己「這句話有沒有讓我想知道答案」，沒有就重下
    - 不得為此堆砌關鍵字（違反第 15 項），也不回頭批次改寫舊文標題（第 17 項），單篇有需要才改
    - 站內落地：skyfaring 的 `CLAUDE.md`〈標題與子標題〉節；標點規則由 `scripts/check-post.mjs` 檢查
